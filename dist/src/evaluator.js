...
Возвращай ТОЛЬКО валидный JSON, соответствующий точной схеме.`,
        ...
        try {
            ...
            const fixedResponse = await openaiClient_1.openai.chat.completions.create({
                model: 'gpt-4-turbo-preview',
                messages: [
                    {
                        role: 'system',
        ...
        const result = types_1.EvaluationResultSchema.parse(parsed);
        return result;
    }
    catch (error) {
...
function buildEvaluationPrompt(steps) {
