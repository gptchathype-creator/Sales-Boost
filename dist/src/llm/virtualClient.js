...
exports.buildDealershipFromCar = buildDealershipFromCar;
exports.getVirtualClientReply = getVirtualClientReply;
...
function buildDealershipFromCar(car) {
    const loc = car.location;
    const terms = car.deal_terms;
...
function parseVirtualClientOutput(raw, currentState) {
    ...
    const diagnostics = {
        ...
        phase_checks_update: diag.phase_checks_update && typeof diag.phase_checks_update === 'object'
    ...
    const client_turns = typeof ct === 'number' && Number.isFinite(ct)
        ? ct
...
function buildFallbackOutput(message, state) {
    return {
...
// ── Main call ──
async function getVirtualClientReply(input) {
    ...
    const profile = input.state.client_profile ?? 'normal';
    const objType = input.state.objection_triggered ?? 'price';
    const profileDesc = (0, clientProfile_1.profileToPromptDescription)(profile);
    ...
    if (beh) {
        ...
        if (beh.evasion)
            flags.push('evasion');
        if (beh.prohibited_phrase_hits.length > 0)
            flags.push('prohibited_phrases');
...
- Return ONLY valid JSON.`;
    ...
    try {
        ...
        if (status === 403 && code === 'unsupported_country_region_territory') {
            throw new Error('OpenAI: API недоступен в вашем регионе. Используйте VPN/прокси: добавьте HTTPS_PROXY в .env.');
        }
        if (status === 403) {
