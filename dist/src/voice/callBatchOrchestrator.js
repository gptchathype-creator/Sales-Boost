...
exports.startCallBatchOrchestrator = startCallBatchOrchestrator;
exports.stopCallBatchOrchestrator = stopCallBatchOrchestrator;
exports.createCallBatch = createCallBatch;
exports.listCallBatches = listCallBatches;
exports.getCallBatch = getCallBatch;
exports.getCallBatchJobs = getCallBatchJobs;
exports.pauseCallBatch = pauseCallBatch;
exports.resumeCallBatch = resumeCallBatch;
exports.cancelCallBatch = cancelCallBatch;
exports.onVoxBatchWebhook = onVoxBatchWebhook;
const db_1 = require("../db");
const callHistory_1 = require("./callHistory");
const startVoiceCall_1 = require("./startVoiceCall");
const dealershipCallSource_1 = require("./dealershipCallSource");
const TICK_MS = 1000;
const DEFAULT_SCENARIO = 'realtime_pure';
const RETRIABLE_OUTCOMES = new Set(['busy', 'no_answer', 'failed']);
const nextDispatchAtByBatch = new Map();
const batchRuntimeConfig = new Map();
let lastAutoDailyKey = null;
let timer = null;
...
function isRetriableOutcome(outcome) {
...
function normalizeWebhookOutcome(event) {
    if (!event)
        return 'disconnected';
    if (event === 'busy' || event === 'no_answer' || event === 'failed')
        return event;
    if (event === 'disconnected')
        return 'disconnected';
    return 'disconnected';
}
function asAuditIdFromVoiceSession(sessionId) {
...
function randomTimesInWorkingWindow(baseDay, startHour, endHour, count) {
...
async function recomputeBatchCounters(batchId) {
    const jobs = await db_1.prisma.callBatchJob.findMany({
        where: { batchId },
        select: { status: true },
    });
    const counts = {
    ...
    for (const j of jobs) {
        if (j.status === 'queued')
            counts.queuedJobs += 1;
        else if (j.status === 'retry_wait')
            counts.retryingJobs += 1;
        else if (j.status === 'dialing' || j.status === 'in_progress')
            counts.inProgressJobs += 1;
        else if (j.status === 'completed')
            counts.completedJobs += 1;
        else if (j.status === 'failed')
            counts.failedJobs += 1;
        else if (j.status === 'cancelled')
            counts.cancelledJobs += 1;
    }
    const batch = await db_1.prisma.callBatch.findUnique({
        where: { id: batchId },
    ...
    await db_1.prisma.callBatch.update({
        where: { id: batchId },
        data: {
            ...counts,
...
async function markJobFailedOrRetry(jobId, outcome, errorText) {
    const job = await db_1.prisma.callBatchJob.findUnique({
        where: { id: jobId },
        include: { batch: true },
    });
    if (!job)
    ...
    if (canRetry) {
        await db_1.prisma.callBatchJob.update({
            where: { id: job.id },
            data: {
                status: 'retry_wait',
...
async function syncLinkedAuditByCallId(jobId, callId, fallbackReason = 'no_call_review_yet') {
    if (!callId)
        return;
    const session = await db_1.prisma.voiceCallSession.findUnique({
        where: { callId },
...
async function dispatchOneJob(batchId, scenario) {
    const batch = await db_1.prisma.callBatch.findUnique({
        where: { id: batchId },
    ...
    if (!batch || batch.status !== 'running')
        return false;
    const active = await db_1.prisma.callBatchJob.count({
        where: {
            batchId,
            status: { in: ['dialing', 'in_progress'] },
        },
    });
    if (active >= batch.maxConcurrency)
        return false;
    const nextAllowedAt = nextDispatchAtByBatch.get(batchId) ?? 0;
    if (Date.now() < nextAllowedAt)
        return false;
    const job = await db_1.prisma.callBatchJob.findFirst({
        where: {
            batchId,
    ...
    if (!job)
        return false;
    const updated = await db_1.prisma.callBatchJob.updateMany({
        where: {
            id: job.id,
            status: { in: ['queued', 'retry_wait'] },
        },
    ...
    if (updated.count === 0)
        return false;
    const fresh = await db_1.prisma.callBatchJob.findUnique({ where: { id: job.id } });
    if (!fresh)
        return false;
    ...
    if (testMode) {
        ...
        await db_1.prisma.callBatchAttempt.create({
            data: {
                jobId: fresh.id,
    ...
    const result = await (0, startVoiceCall_1.startVoiceCall)(fresh.phone, { scenario });
    if ('error' in result) {
        await markJobFailedOrRetry(fresh.id, 'failed', result.error);
        return true;
    }
    (0, callHistory_1.addCall)(result.callId, fresh.phone);
    await db_1.prisma.callBatchAttempt.create({
        data: {
            jobId: fresh.id,
            attempt: fresh.attempt,
...
async function processTick() {
    ...
    tickInFlight = true;
    try {
        await db_1.prisma.callBatchJob.updateMany({
            where: {
                status: 'retry_wait',
                nextRunAt: { lte: now() },
            },
        ...
        const runningBatches = await db_1.prisma.callBatch.findMany({
            where: { status: 'running' },
            select: { id: true },
        });
        for (const b of runningBatches) {
            const runtime = batchRuntimeConfig.get(b.id);
            const scenario = runtime?.scenario || (process.env.CALL_BATCH_SCENARIO || DEFAULT_SCENARIO);
...
async function reapStuckInProgressJobs() {
    const staleBefore = new Date(Date.now() - timeoutMs());
    const staleJobs = await db_1.prisma.callBatchJob.findMany({
        where: {
            status: 'in_progress',
    ...
    for (const j of staleJobs) {
        await db_1.prisma.callBatchAttempt.updateMany({
...
async function maybeCreateAutoDailyBatch() {
    ...
    if (lastAutoDailyKey === dayKey)
        return;
    const existing = await db_1.prisma.callBatch.count({
        where: {
            mode: 'auto_daily',
            createdAt: {
    ...
    const dealerships = (0, dealershipCallSource_1.listDealershipCallTargets)();
...
function startCallBatchOrchestrator() {
    if (timer)
        return;
    timer = setInterval(() => {
...
function stopCallBatchOrchestrator() {
    if (!timer)
        return;
    clearInterval(timer);
    timer = null;
}
async function createCallBatch(input) {
    ...
    const scenario = input.scenario ?? (process.env.CALL_BATCH_SCENARIO || DEFAULT_SCENARIO);
    const isManualMode = mode === 'manual' || mode === 'single_dealership' || mode === 'all_dealerships';
    if (isManualMode) {
        const activeManual = await db_1.prisma.callBatch.count({
            where: {
    ...
    const batch = await db_1.prisma.callBatch.create({
        data: {
            mode,
            title,
            status: 'running',
...
async function listCallBatches(limit, mode) {
    const rows = await db_1.prisma.callBatch.findMany({
...
async function getCallBatch(batchId) {
    const batch = await db_1.prisma.callBatch.findUnique({
        where: { id: batchId },
    });
    if (!batch) {
    ...
    const jobsPreview = await db_1.prisma.callBatchJob.findMany({
        where: { batchId },
        orderBy: [{ queuedAt: 'asc' }],
        take: 100,
    });
    const jobsForSummary = await db_1.prisma.callBatchJob.findMany({
        where: { batchId },
        select: {
            dealershipId: true,
            dealershipName: true,
    ...
    const dealershipSummary = [...summaryMap.values()].map((s) => {
        let status = 'queued';
        if (s.inProgress > 0 || s.retrying > 0)
            status = 'in_progress';
        else if (s.failed > 0 && s.completed > 0)
            status = 'partial';
        else if (s.failed > 0)
            status = 'failed';
        else if (s.completed === s.total)
            status = 'completed';
        else if (s.cancelled === s.total)
            status = 'cancelled';
        return { ...s, status };
    });
    return { batch, jobsPreview, dealershipSummary };
}
async function getCallBatchJobs(batchId, limit, offset, status) {
    const where = {
        batchId,
        ...(status ? { status } : {}),
    };
    const [jobsRaw, total] = await Promise.all([
        db_1.prisma.callBatchJob.findMany({
            where,
            orderBy: [{ queuedAt: 'asc' }],
            take: limit,
            skip: offset,
        }),
        db_1.prisma.callBatchJob.count({ where }),
    ]);
    const callIds = [...new Set(jobsRaw.map((j) => j.lastVoiceCallId).filter((v) => !!v))];
    const sessions = callIds.length
        ? await db_1.prisma.voiceCallSession.findMany({
    ...
    for (const j of jobsRaw) {
        if (j.linkedAuditId || !j.lastVoiceCallId)
            continue;
...
async function pauseCallBatch(batchId) {
    await db_1.prisma.callBatch.update({
        where: { id: batchId },
        data: { status: 'paused' },
    });
}
async function resumeCallBatch(batchId) {
    await db_1.prisma.callBatch.update({
        where: { id: batchId },
        data: { status: 'running' },
    });
}
async function cancelCallBatch(batchId) {
    await db_1.prisma.callBatch.update({
        where: { id: batchId },
...
async function onVoxBatchWebhook(payload) {
    const callId = payload.call_id;
    if (!callId)
        return;
    const attempt = await db_1.prisma.callBatchAttempt.findUnique({
    ...
    await db_1.prisma.callBatchAttempt.update({
        where: { id: attempt.id },
        data: {
            status: 'ended',
            outcome,
    ...
    if (canRetry) {
        ...
        const finalStatus = outcome === 'busy' || outcome === 'no_answer' || outcome === 'failed'
            ? 'failed'
            : 'completed';
        await db_1.prisma.callBatchJob.update({
            where: { id: job.id },
...
//# sourceMappingURL=callBatchOrchestrator.js.map