...
function isLikelyDisengaging(text) {
    const direct = containsAny(text, DISENGAGING_PATTERNS);
    if (direct.length > 0)
        return true;
    const stopAndTalkRegex = /(не\s*(хочу|буду).*(разговар|говор|общат))|((stop|dont|don't)\s+(call|talk|message))/i;
    return stopAndTalkRegex.test(text);
}
// ── Main classifier ──
function classifyBehavior(managerMessage, context) {
    ...
    if (hostileHits.length > 0)
        reasons.push('hostile language');
    if (disengaging)
        reasons.push('conversation shutdown / refusal intent');
    ...
    if (prohibitedHits.length > 0)
        reasons.push(`prohibited phrases: ${prohibitedHits.join(', ')}`);
    ...
    if (isVeryShort)
        reasons.push(`very short (${charCount} chars, ${wordCount} words)`);
    if (isNonsense)
        reasons.push('nonsense/slang token');
    if (isOnlyFiller)
        reasons.push('only filler words');
    // ── Evasion detection ──
    let evasion = false;
    if (context.isClientWaitingAnswer) {
        const answersNothing = isLowEffort ||
            disengaging ||
    ...
    if (lowQuality && !disengaging)
        reasons.push('low-quality / disengaged answer');
    // ── Severity ──
    let severity = 'LOW';
...
//# sourceMappingURL=behaviorClassifier.js.map