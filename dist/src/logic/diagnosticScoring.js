...
exports.ISSUE_TYPES = exports.CHECKLIST_WEIGHTS = exports.CHECKLIST_CODE = void 0;
exports.computeDeterministicScore = computeDeterministicScore;
exports.buildChecklistFromLLMClassification = buildChecklistFromLLMClassification;
exports.detectIssuesFromChecklist = detectIssuesFromChecklist;
exports.CHECKLIST_CODE = [
    'INTRODUCTION',
    'SALON_NAME',
    'CAR_IDENTIFICATION',
    'NEEDS_DISCOVERY',
...
function computeDimensionScore(checklist, codes) {
...
function computeDeterministicScore(checklist, options) {
...
function buildChecklistFromLLMClassification(classification) {
    return exports.CHECKLIST_CODE.map((code) => {
        const match = classification.find((c) => c.code === code);
        const weight = exports.CHECKLIST_WEIGHTS[code];
        if (!match) {
            return { code, weight, status: 'NA', evidence: [], comment: '' };
        }
...
function detectIssuesFromChecklist(checklist, extraSignals) {
    const issues = [];
    const find = (code) => checklist.find((c) => c.code === code);
...
//# sourceMappingURL=diagnosticScoring.js.map